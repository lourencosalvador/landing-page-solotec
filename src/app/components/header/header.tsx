  ...
  return (
    <header className=" relative z-10 w-full h-20   flex items-center mt-[20px] mb-[95px]">
      <GridContainer className="lg:px-3 px-5 ">
        ...
        <List<HeaderLinks>
          items={HeaderLinksData}
          classeName="lg:flex hidden gap-7 justify-center ml-16 items-center"
        >
          {({ item }) => (
            <li
              key={item.route}
              className={`text-[12.5px] text-[#B0B0B0]  ${
                item.state &&
                "bg-[#FFFFFF40] py-[6px] px-[0.70rem] rounded-md font-semibold  text-[#ffff]"
              }`}
            >
        ...
        <div className="btns flex justify-center items-center gap-[10px]">
          ...
          <Buttom title="Join us" className="bg-[#FEB548] hover:bg-[#d7993d]" />
        </div>
      </GridContainer>
    </header>
  );
